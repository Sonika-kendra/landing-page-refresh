...
const defaultValues: FilterValues = {
  shape: '', colour: '', clarity: '', cut: '', fluorescence: '',
  price: DEFAULT_PRICE_RANGE, caratWeight: DEFAULT_CARAT_RANGE, certificate: '',
};
...
const staticOptions: Record<string, string[]> = {
  ...
  cut: CUTS, fluorescence: FLUORESCENCES, certificate: CERTIFICATES,
};
...
const renderTab = (tab: { key: TabKey; label: string }, value: FilterValues[TabKey], onChange: (k: TabKey, v: unknown) => void) => {
  ...
  if (tab.key === 'price') {
    const presets: { label: string; value: [number, number] }[] = [
      ...
      { label: '£10,000+',           value: [10000, 50000] },
      { label: 'All Prices',         value: DEFAULT_PRICE_RANGE },
    ];
    return (
      <div className="space-y-0.5">
        {presets.map((p) => {
          const isActive = isSameRange(value as [number, number], p.value);
          return (
            <button
              key={p.label} type="button"
              onClick={() => onChange(tab.key, p.value)}
  ...
  // Pill buttons for shape, colour, clarity, cut, fluorescence, certificate
...
const DiamondShopPage = () => {
  ...
  useEffect(() => {
    ...
    if (!isSameRange(filterValues.price, DEFAULT_PRICE_RANGE)) {
      const [lo, hi] = filterValues.price;
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      filterValues.cut, filterValues.fluorescence, filterValues.certificate,
      filterValues.price, filterValues.caratWeight]);
  ...
  const setFilter = useCallback((key: TabKey, value: unknown) => {
    ...
    if (key === 'price') {
      ...
      lo > DEFAULT_PRICE_RANGE[0] ? newParams.set('price_min', String(lo)) : newParams.delete('price_min');
      hi < DEFAULT_PRICE_RANGE[1] ? newParams.set('price_max', String(hi)) : newParams.delete('price_max');
    } else if (key === 'caratWeight') {
      const [lo, hi] = value as [number, number];
      lo > DEFAULT_CARAT_RANGE[0] ? newParams.set('carat_min', String(lo)) : newParams.delete('carat_min');
      hi < DEFAULT_CARAT_RANGE[1] ? newParams.set('carat_max', String(hi)) : newParams.delete('carat_max');
    } else {
      write(key, value);
    }
  ...
  const hasActiveFilters = useMemo(() => {
    return Boolean(
      ...
      filterValues.shape || filterValues.colour || filterValues.clarity ||
      filterValues.cut || filterValues.fluorescence || filterValues.certificate ||
      !isSameRange(filterValues.price, DEFAULT_PRICE_RANGE) ||
  ...
  const activeChips = useMemo(() => {
    ...
    if (!isSameRange(filterValues.price, DEFAULT_PRICE_RANGE)) {
      const [lo, hi] = filterValues.price;
      chips.push({ key: 'price', label: `£${lo.toLocaleString()}–£${hi.toLocaleString()}` });
    }
    if (!isSameRange(filterValues.caratWeight, DEFAULT_CARAT_RANGE)) {
      const [lo, hi] = filterValues.caratWeight;
                ...
                <div className="flex-1 overflow-y-auto px-6 py-6">
                  ...
                  <Accordion type="multiple" value={openAccordion} onValueChange={setOpenAccordion} className="space-y-2">
                    {filterTabs.map((tab) => {
                      ...
                      const isActive =
                        Boolean(val) &&
                        !(tab.key === 'price' && isSameRange(val as [number, number], DEFAULT_PRICE_RANGE)) &&
                        !(tab.key === 'caratWeight' && isSameRange(val as [number, number], DEFAULT_CARAT_RANGE));
                          ...
                          <AccordionContent className="pb-4 pt-0">
                            {isActive && (
                              <div className="mb-3 flex justify-end">
                                <button
                                  type="button"
                                  onClick={() => setFilter(tab.key, tab.key === 'price' ? DEFAULT_PRICE_RANGE : tab.key === 'caratWeight' ? DEFAULT_CARAT_RANGE : '')}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {activeChips.map((chip) => (
                <button
                  ...
                  onClick={() => setFilter(chip.key, chip.key === 'price' ? DEFAULT_PRICE_RANGE : chip.key === 'caratWeight' ? DEFAULT_CARAT_RANGE : '')}
